...
const nullIfBlank = (s?: string | null) => {
    if (s == null) return null;
    const t = s.trim();
    return t.length ? t : null;
};

function extractProblemDetails(err: any): { title?: string; errors?: Record<string, string[]> } {
    const data = (err && (err.response?.data ?? err.data)) ?? null;
    if (data && typeof data === "object") {
        const title = data.title ?? data.detail ?? data.error ?? undefined;
    ...
    try {
        ...
        return { title: parsed.title ?? parsed.detail, errors: parsed.errors };
    } catch {
        return { title: msg || "Der opstod en fejl.", errors: undefined };
    }
}
function mapModelStateErrors(errors?: Record<string, string[]>) {
    const out: { name?: string; email?: string; phone?: string; general?: string } = {};
    if (!errors) return out;
    const first = (k: string) => {
        const v = errors[k];
        return Array.isArray(v) && v.length ? v[0] : undefined;
    };
    out.name = first("Name") ?? first("name");
    out.email = first("Email") ?? first("email");
    out.phone = first("Phone") ?? first("phone");
    
    const known = new Set(["Name", "name", "Email", "email", "Phone", "phone"]);
    const extras = Object.entries(errors)
        .filter(([k]) => !known.has(k))
        .flatMap(([, arr]) => arr)
        .filter(Boolean);
    if (extras.length) out.general = extras[0];
    return out;
}
...
type GameDTO = {
    ...
    status?: "inactive" | "active" | "closed" | string; Status?: string;
    winning?: number[] | null;
    winning_nums?: number[] | null;
    revenueDkk?: number; revenue_dkk?: number; RevenueDkk?: number;
};
    ...
    const active =
        typeof p.is_active === "boolean" ? p.is_active :
            typeof p.isActive === "boolean" ? p.isActive :
                !!p.active;
    return { id: p.id, name: p.name, email: p.email, phone: p.phone ?? undefined, active };
}
function mapGame(g: GameDTO): GameUI {
    const nums = pickArray<number>(g.winning ?? g.winning_nums);
    const weekStart = g.week_start ?? g.weekStart;
    const status = g.status ?? g.Status;
    const revenue = g.revenueDkk ?? g.revenue_dkk ?? g.RevenueDkk ?? 0;
    return { id: g.id, week_start: weekStart, status, winning: nums, revenueDkk: Number(revenue) || 0 };
}
...
export default function AdminDashboard() {
    ...
    const displayWeekYear = useMemo(() => {
        if (activeGame?.week_start) return toWeekYear(activeGame.week_start);
        return { week: currentWeek, year: currentYear };
    }, [activeGame?.week_start, currentWeek, currentYear]);
    
    const [saving, setSaving] = useState(false);
    const [saveErr, setSaveErr] = useState<string | null>(null);
    const [formErrors, setFormErrors] = useState<{ name?: string; email?: string; phone?: string; general?: string }>({});
    ...
    function validateClient(f: { name: string; email: string; phone: string }) {
        const fe: typeof formErrors = {};
        if (f.name.trim().length < 2) fe.name = "Navn skal være mindst 2 tegn.";
        if (f.phone.trim() && !/^[+0-9()\s-]{6,20}$/.test(f.phone.trim())) fe.phone = "Indtast et gyldigt telefonnummer.";
        return fe;
    }

    async function savePlayer(
        edit?: PlayerUI,
        f?: { name: string; email: string; phone: string; active: boolean }
    ) {
        if (!f) return;
        resetFormErrors();
        const fe = validateClient(f);
        ...
        try {
            if (edit) {
                await http.put(`/players/${edit.id}`, {
                    Name: f.name.trim(),
            ...
            const pd = extractProblemDetails(e);
            const fields = mapModelStateErrors(pd.errors);
            setFormErrors(fields);
            if (pd.title && !fields.general && !fields.name && !fields.email && !fields.phone) {
                setSaveErr("Kunne ikke gemme – tjek felterne.");
            }
        } finally {
            setSaving(false);
        }
    ...
    async function toggleActive(id: number) {
        ...
        try {
            await http.put(`/players/${id}`, {
                Name: p.name,
                Email: nullIfBlank(p.email ?? null),
                Phone: nullIfBlank(p.phone ?? null),
                IsActive: !p.active,
                MemberExpiresAt: null,
            });
            await fetchPlayers();
            ...
            const fields = mapModelStateErrors(pd.errors);
            setFormErrors(fields);
            setSaveErr(pd.title ?? "Kunne ikke opdatere status.");
        }
    }
    ...
    async function saveWinners() {
        if (!hasActive || !targetGame || picks.length !== WIN_COUNT) return;
        ...
        <div className="page admin-page" style={{ paddingBottom: 40 }}>
            ...
            <div className="admin-card">
                ...
                {tab === "history" && (
                    <section>
                        ...
                        <div className="admin-table">
                            <div className="admin-thead"><div>Uge</div><div>År</div><div>Status</div><div>Omsætning</div><div>Vindertal</div><div></div></div>
                            {combinedGames.map((g) => {
                                    ...
                                    <div key={g.id} style={{ display: "contents" }}>
                                        <div className="admin-trow">
                                            ...
                                            <div style={{ justifySelf: "end" }}>
                                                <button className="balance-btn" onClick={() => { const next = isOpen ? null : g.id!; setOpenGameId(next); if (next) fetchBoardsFor(next); }}>
                                                ...
                                                {sheet && (
                                                    <>
                                                        <div className="admin-row" style={{ marginBottom: 6 }}>
                                                            <div className="admin-muted">{sheet.boardsTotal} bræt · {sheet.winnersTotal} vinder-bræt</div>
                                                        </div>
                                                        <div className="admin-table">
                                                            <div className="admin-thead"><div>Bræt #</div><div>Spiller</div><div>Tal</div><div>Pris</div><div>Vinder</div></div>
                                                            {rows.map((b) => {
                                                                const winNums = nums;
                                                                return (
                                                                        ...
                                                                        <div>
                                                                            <div className="wins" style={{ gap: 6 }}>
                                                                                {pickArray<number>(b.numbers).map((n) => (
                                                                                    <span key={n} className={`win-ball ${winNums.includes(n) ? "" : "win-ball--muted"}`} title={winNums.includes(n) ? "Match" : ""}>{n}</span>
                                                                                ))}
                                                                            </div>
                                                                        </div>
                                                                        <div>{b.priceDkk} DKK</div>
                                                                        <div><span className={`chip ${b.isWinner ? "chip--ok" : "chip--muted"}`}>{b.isWinner ? "Vinder" : "–"}</span></div>
                                                                    </div>
                                                                );
                                                            })}
                                                            {rows.length === 0 && <div className="admin-trow"><div>Ingen brætter.</div></div>}
                                                        </div>
                                                    </>
                                                )}
                                            </div>
                            ...
                            {combinedGames.length === 0 && (<div className="admin-trow"><div>–</div><div>–</div><div>Ingen data</div><div>–</div><div>–</div></div>)}
                        </div>
                    </section>
                )}
                ...
                {tab === "winners" && (
                    <section>
                        <ThisWeekWinners
                            ...
                            week={displayWeekYear.week}
                            year={displayWeekYear.year}
                        />
                ...
                <span className="admin-muted">
                  {hasActive ? (() => { const wy = toWeekYear(targetGame!.week_start); return `Vælg ${WIN_COUNT} vindertal for uge ${wy.week} · ${wy.year}`; })() : "Ingen aktiv uge — den oprettes automatisk efter sidste udtræk."}
                </span>
                            </div>
                        ...
                        <div className="picker-grid" role="group" aria-label="Vælg vindertal 1–16">
                            {Array.from({ length: 16 }, (_, i) => i + 1).map((n) => {
                                ...
                                return (
                                    <button key={n} type="button" className={`pick-ball ${active ? "pick-ball--active" : ""}`} onClick={() => togglePick(n)} aria-pressed={active} disabled={!hasActive}>
                                        <span>{n}</span>
                                    </button>
                                );
                            })}
                            ...
                            {txs.map((tx) => {
                                ...
                                return (
                                    <div className="admin-trow" key={tx.id}>
                                        ...
                                        <div className="admin-actions">
                                            <button className="balance-btn" onClick={() => http.post("/transactions/decide", { transactionId: tx.id, decision: "approve" }).then(fetchTransactions)}>Godkend</button>
                                            <button className="danger-btn" onClick={() => http.post("/transactions/decide", { transactionId: tx.id, decision: "reject" }).then(fetchTransactions)}>Afvis</button>
                                        </div>
                                    </div>
                                );
                            })}
                    ...
                    <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="admin-modal__body">
                            ...
                            <label className="admin-field">Navn
                                ...
                                {formErrors.name && <div style={{ color: "#f88", fontSize: 12, marginTop: 4 }}>{formErrors.name}</div>}
                            </label>
                            ...
                            <label className="admin-field">E-mail
                                ...
                                {formErrors.email && <div style={{ color: "#f88", fontSize: 12, marginTop: 4 }}>{formErrors.email}</div>}
                            </label>
                            ...
                            <label className="admin-field">Telefon
                                ...
                                {formErrors.phone && <div style={{ color: "#f88", fontSize: 12, marginTop: 4 }}>{formErrors.phone}</div>}
                            </label>
                            ...
                            {formErrors.general && <div style={{ color: "#f88", fontSize: 12, marginTop: 8 }}>{formErrors.general}</div>}
                        </div>
